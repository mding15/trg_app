"""
backfill_concentrations.py — Backfill db_concentrations from position_var data.

Iterates over all (account_id, as_of_date) pairs in position_var that fall
within the given date range and writes concentration rows to db_concentrations.
Skips dates that already have rows unless --overwrite is specified.

Usage
-----
    # all accounts, all dates from 2026-04-21 to today
    python dashboard/backfill_concentrations.py --from-date 2026-04-21

    # single account, date range
    python dashboard/backfill_concentrations.py --account-id 1001 \
        --from-date 2026-01-01 --to-date 2026-04-21

    # overwrite existing rows
    python dashboard/backfill_concentrations.py --from-date 2026-04-21 --overwrite
"""
...
def _already_exists(account_id: int, as_of_date: datetime.date) -> bool:
    """Return True if db_concentrations already has rows for (account_id, as_of_date)."""
    with pg_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT 1 FROM db_concentrations WHERE account_id = %s AND as_of_date = %s LIMIT 1",
                (account_id, as_of_date),
            )
            return cur.fetchone() is not None


def backfill_concentrations(
    from_date: datetime.date,
    to_date: datetime.date,
    ...
    for acct, as_of in pairs:
        ...
        try:
            ...
            rows = compute_concentrations(acct, as_of, df=df, limits=limits)
            delete_concentrations(acct, as_of)
            write_concentrations(acct, as_of, rows)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Backfill db_concentrations from position_var data."
    )
