# -*- coding: utf-8 -*-
"""
Created on Wed Sep  4 17:21:27 2024
...
TABLE_FILE = config['SEC_DIR'] / 'fund_sectors.csv'
TABLE_COLUMNS = ['SecurityID','Sector', 'Weight', 'UpdateDate']
...
FUND_SECTOR_XL_FILE = config['HOME_DIR'].parent / 'Models' / 'IndustrySector' / 'FundSectors.xlsx'
def view():
...
def xl_add_new():
    ...
    df = xl_utils.read_df_from_excel(wb, 'Sectors')    
    ...
    df = df.groupby(by=['SecurityID', 'Sector'])['Weight'].sum()
    df = df.reset_index()    
