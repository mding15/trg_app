# -*- coding: utf-8 -*-
"""
Created on Wed Sep 11 09:39:29 2024
...
TABLE_FILE = config['SEC_DIR'] / 'fund_regions.csv'
TABLE_COLUMNS = ['SecurityID','Region', 'Weight', 'UpdateDate']
...
FUND_SECTOR_XL_FILE = config['HOME_DIR'].parent / 'Models' / 'IndustrySector' / 'FundRegions.xlsx'
def view():
    wb = xw.Book(FUND_SECTOR_XL_FILE)
    df = get()    
    ...
    xl_utils.add_df_to_excel(df, wb, 'Region', index=False)    
    
    
def xl_add_new():
    wb = xw.Book(FUND_SECTOR_XL_FILE)
    df = xl_utils.read_df_from_excel(wb, 'temp')    
    
    # make sure weights add up to 100%
    tw = df.groupby(by=['SecurityID'])['Weight'].sum()
    bad = tw[(tw>1.02) | (tw<0.98)]
    if len(bad)>0:
    ...
    df = df.groupby(by=['SecurityID', 'Region'])['Weight'].sum()
    df = df.reset_index()    
...
def test():
    ...
    new_data = xl_utils.read_df_from_excel(wb, 'FundSectors')    
