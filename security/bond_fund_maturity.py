# -*- coding: utf-8 -*-
"""
Created on Sat Sep 21 16:06:16 2024
...
TABLE_FILE = config['SEC_DIR'] / 'bond_fund_maturity.csv'
TABLE_COLUMNS = ['SecurityID','Maturity', 'Weight', 'UpdateDate']
...
BOND_FUND_MATURITY_XL_FILE = config['HOME_DIR'].parent / 'Models' / 'IndustrySector' / 'BondFundMaturity.xlsx'
def view():
    wb = xw.Book(BOND_FUND_MATURITY_XL_FILE)
    df = get()    
    ...
    xl_utils.add_df_to_excel(df, wb, 'FundMaturity', index=False)    
...
def xl_add_new():
    ...
    df = df.groupby(by=['SecurityID', 'Maturity'])['Weight'].sum()
    df = df.reset_index()    
